...
_norm_rope_kernels: dict[int, Any] = {}


def _get_norm_rope_kernel(head_dim: int, eps: float = 1e-6, rope_base: float = 1000000.0) -> Any:
    if head_dim in _norm_rope_kernels:
        return _norm_rope_kernels[head_dim]
    ...
    _norm_rope_kernels[head_dim] = kernel
    return kernel


def fused_norm_rope(proj_out: mx.array, norm_weight: mx.array, num_heads: int, head_dim: int, offset: int) -> mx.array:
    b, s, _ = proj_out.shape
    kernel = _get_norm_rope_kernel(head_dim)
    return kernel(
...
def baseline_norm_rope(
    proj_out: mx.array, norm_weight: mx.array, eps: float, num_heads: int, head_dim: int, offset: int
) -> mx.array:
    b, s, _ = proj_out.shape
    x = proj_out.reshape(b, s, num_heads, head_dim).transpose(0, 2, 1, 3)
    x = mx.fast.rms_norm(x, norm_weight, eps)
    return mx.fast.rope(x, head_dim, traditional=False, base=1000000.0, scale=1.0, offset=offset)
...
class Attention(nn.Module):
    ...
    def __call__(
        ...
        q = fused_norm_rope(self.q_proj(x), self.q_norm.weight, self.num_heads, self.head_dim, offset)
        k = fused_norm_rope(self.k_proj(x), self.k_norm.weight, self.num_kv_heads, self.head_dim, offset)
